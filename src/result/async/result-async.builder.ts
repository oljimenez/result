import { type Result, errSync, okSync } from "../sync/result.builder";
...
class ResultAsync<O, E> implements PromiseLike<Result<O, E>> {
	...
	andThen<TOk, TErr>(
		fn: (t: O) => ResultAsync<TOk, TErr>,
	): ResultAsync<TOk, E | TErr>;
	andThen(
		fn: (t: O) => ResultAsync<unknown, unknown>,
	...
	orElse<TOk, TErr>(
		fn: (t: E) => ResultAsync<TOk, TErr>,
	): ResultAsync<O | TOk, TErr>;
	orElse(
		fn: (t: E) => ResultAsync<unknown, unknown>,
	): ResultAsync<unknown, unknown> {
		return new ResultAsync(
			this._promise.then((res) => {
				return res.match({
	...
	>(
		...
		...args: TArgs
	) => ResultAsync<InferAsyncOkTypes<TResult>, InferAsyncErrTypes<TResult>>;
	static infer<TArgs extends any[], TResult extends Result<unknown, unknown>>(
		fn: (...args: TArgs) => Promise<TResult>,
	): (
		...args: TArgs
	) => ResultAsync<InferOkTypes<TResult>, InferErrTypes<TResult>>;
